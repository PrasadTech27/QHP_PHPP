...
// Fetch Primary Delivery Address Safely using global $conn
$primary = null;
...
// Fetch Admin Added Medicines using global $conn
$medicines_result = $conn->query("SELECT * FROM medicines ORDER BY id DESC");
?>
...
<div class="med-wrapper">
    ...
    <div class="med-search-bar">
        ...
        <input type="text" id="medSearchInput" oninput="filterMedicines()" placeholder="Search medicines by name or description...">
    </div>

    <div style="font-size:18px; font-weight:800; color:#0D47A1; margin-bottom:16px;">
        <i class="fas fa-capsules" style="margin-right:6px;"></i> Available Medicines
    </div>

    <div class="med-grid" id="medicinesContainerGrid">
        <?php if ($medicines_result && $medicines_result->num_rows > 0): ?>
            <?php while($m = $medicines_result->fetch_assoc()): 
                $img = !empty($m['image_url']) ? $m['image_url'] : 'https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?w=300';
                $safeName = htmlspecialchars($m['name'], ENT_QUOTES);
            ?>
                <div class="med-item-card" data-medname="<?= htmlspecialchars(strtolower($m['name'])) ?>" data-meddesc="<?= htmlspecialchars(strtolower($m['description'])) ?>">
                    <div class="med-img-box">
                        <img src="<?= htmlspecialchars($img) ?>" alt="Medicine">
                    </div>
                    <div style="padding:15px; display:flex; flex-direction:column; justify-content:space-between; flex:1;">
                        <div>
                            <strong style="font-size:16px; color:#1e293b; display:block; margin-bottom:4px;"><?= htmlspecialchars($m['name']) ?></strong>
                            <p style="color:#64748b; font-size:12px; line-height:1.3; margin-bottom:10px;"><?= htmlspecialchars($m['description']) ?></p>
                        </div>
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            <span style="font-size:17px; font-weight:900; color:#0D47A1;">₹<?= $m['price'] ?></span>
                            <div class="dynamic-action-box">
                                <button class="btn-add-cart" onclick="updateMedicineQty('<?= $safeName ?>', <?= $m['price'] ?>, 1)">ADD +</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <?php else: ?>
            <div style="text-align:center; grid-column:1/-1; padding:45px 20px; color:#64748b; background:#fff; border-radius:20px; border:1px solid #e2e8f0;">
                <i class="fas fa-pills" style="font-size:42px; color:#cbd5e1; margin-bottom:12px;"></i>
                <h3 style="color:#1e293b; margin-bottom:6px;">No Medicines Available!</h3>
                <p style="font-size:13px;">No medicines found in the inventory right now.</p>
            </div>
        <?php endif; ?>
    </div>
</div>
...
function updateMedicineQty(medName, medPrice, change) {
    let cart = getCartData();
    let existing = cart.find(i => i.name === medName);

    if (existing) {
        existing.qty += change;
        if (existing.qty <= 0) {
            cart = cart.filter(i => i.name !== medName);
        }
    } else if (change > 0) {
        cart.push({
            name: medName,
...
function renderMedicineButtons() {
    ...
    cards.forEach(card => {
        ...
        if (!nameEl) return;
        const medName = nameEl.innerText.trim();
        const priceText = card.querySelector('span[style*="color:#0D47A1"]').innerText.replace('₹', '');
        const medPrice = Number(priceText);
        
        let container = card.querySelector('.dynamic-action-box');
        if (!container) return;

        let cartItem = cart.find(i => i.name === medName);
        if (cartItem && cartItem.qty > 0) {
            container.innerHTML = `
                <div class="qty-stepper">
                    <button type="button" onclick="updateMedicineQty('${medName.replace(/'/g, "\\'")}', ${medPrice}, -1)">-</button>
            ...
            container.innerHTML = `
                <button class="btn-add-cart" onclick="updateMedicineQty('${medName.replace(/'/g, "\\'")}', ${medPrice}, 1)">ADD +</button>
            `;
        }
    });
}
...
function filterMedicines() {
    ...
    cards.forEach(card => {
        const name = card.dataset.medname || '';
        const desc = card.dataset.meddesc || '';
